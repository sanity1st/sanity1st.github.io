...
ei_parse_hints:
  ...
  - critical_avoidances: ["single_test_reliance", "hierarchical_inversion", "logic_without_facts", "laws_without_ethics", "premature_certainty", "treating_eventually_as_a_schedule", "preference_presented_as_finding"]
related_files:
...
* **Divergence on Ethics** — the validators disagree about what Ethics requires. Here a crucial sub-distinction governs everything. *First ask:* is this a real divergence at all, or is one side confusing a tribal preference for a universal principle? The most common Ethics "disagreement" is not a real one — it is proximity bias or social-layer norm masquerading as universal-layer ethics, and it dissolves the moment it is named (see "When Tests Seem to Conflict," above). *But* where the divergence survives that test — where each side's position is genuinely defensible under reciprocity, universality, and non-instrumental regard — something more precise has happened than two parties holding different ethics. **Ethics has been satisfied by more than one option**, and what remains between them is preference. *Route:* this is a **Split Verdict**, and it is where the framework hands from the truth engine to the consent engine (see The Six Verdicts, below). What matters at this stage is only that the Agora recognize the boundary when it reaches it, rather than forcing a false unanimity or letting a genuine plurality of preference masquerade as one side's factual error.
...
One caution attaches to Fail more than to any other verdict. A judgment deployed without actually passing the Gate is a **counterfeit verdict** — the form of a verdict without the work behind it. The temptation is sharpest where the framework's own vocabulary is available: calling a rival position a sealed terminus, a false light, or a captured chain *without running the tests* is itself a counterfeit verdict, and it does precisely what the framework exists to forbid. A counterfeit verdict can protect a counterfeit terminus, which is often what it is for — but neither is diagnosed by assertion.
...
**Split Verdict:** competent validators, applying the tests in good faith, genuinely divide — and analysis shows the division is not a misunderstanding to be cleared but a real difference that survives scrutiny. A Split Verdict is not a failure of the Agora; it is the Agora accurately reporting that more than one defensible option remains after the misaligned ones have been eliminated.

Its proper handling depends on *which test* the validators divide on (see "Routing a Disagreement," above). Splits on Facts or Logic usually resolve with better evidence or a closer look at the inference. But where the split survives Ethics — where each remaining option passes reciprocity, universality, and non-instrumental regard — the situation is not a disagreement about ethics at all. **Ethics has cleared more than one option, and the difference between them is preference.** That is the designed handoff: the truth engine has done its work by eliminating what fails and confirming what remains is defensible; the consent engine chooses among what is left, because selecting among genuinely tied aligned options is a matter of legitimate consent rather than further truth-tracking.

The Agora's job is to narrow the space to the defensible options and to *name the split honestly* — never to manufacture a unanimity the evidence does not support, and never to let preference be presented as though it were a finding.
...
**Detecting a terminus that has sealed.** That last sentence names the failure but not how anyone would catch it, and the catching is harder than it sounds — a sealed terminus does not announce itself, and from inside the trust the source one would consult is the source in question. The check that works has to be runnable from outside, by someone who has not accepted the framework's vocabulary and has no stake in its verdicts.
